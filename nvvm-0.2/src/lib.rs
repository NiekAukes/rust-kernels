...
pub fn ir_version() -> (i32, i32) {
    unsafe { sys::nvvmIRVersion().expect("failed to get nvvm ir version") }
}

/// Get the major and minor NVVM debug metadata version.
pub fn dbg_version() -> (i32, i32) {
    unsafe { sys::nvvmIRDebugVersion().expect("failed to get nvvm ir debug version") }
}

/// Get the major and minor NVVM version.
pub fn nvvm_version() -> (i32, i32) {
    unsafe { sys::nvvmVersion().expect("failed to get nvvm version") }
}

pub fn get_error_string(err: NvvmError) -> String {
    unsafe { sys::nvvmGetErrorString(err.to_raw()) }
}
...
impl Display for NvvmError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        unsafe {
            let str = sys::nvvmGetErrorString(self.to_raw());
            f.write_str(&str)
        }
    }
}
...
impl Drop for NvvmProgram {
    fn drop(&mut self) {
        unsafe {
            sys::nvvmDestroyProgram(&mut self.raw).expect("failed to destroy nvvm program");
        }
    }
}
...
impl NvvmProgram {
    ...
    pub fn new() -> Result<Self, NvvmError> {
        unsafe {
            let raw = match sys::nvvmCreateProgram() {
                Ok(raw) => raw,
                Err(err) => {
                    return Err(NvvmError::from_raw(err));
                }
            };
            Ok(Self { raw })
        }
    ...
    pub fn compile(&self, options: &[NvvmOption]) -> Result<Vec<u8>, NvvmError> {
        unsafe {
            ...
            sys::nvvmCompileProgram(self.raw, options_ptr).to_result()?;
            let mut size = 0;
